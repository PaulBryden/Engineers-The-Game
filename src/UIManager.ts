...
class UIManager
{
    ...
    constructor(scene:Phaser.Scene, entityScene:GameScene, initialEntity?: Entity)
    {
        ...
        scene.add.image(180,250,"Up_Button").setScrollFactor(0).setScale(2.5).setDepth(250).setInteractive().on('pointerdown', (pointer, localX, localY, event)=>{
            this.UpArrowDown=true;
        }).on('pointerup', (pointer, localX, localY, event)=>{
            this.UpArrowDown=false;
        });
        scene.add.image(180,340,"Down_Button").setScrollFactor(0).setScale(2.5).setDepth(250).setInteractive().on('pointerdown', (pointer, localX, localY, event)=>{
            this.BottomArrowDown=true;
        }).on('pointerup', (pointer, localX, localY, event)=>{
            this.BottomArrowDown=false;
        });
        scene.add.image(125,295,"Left_Button").setScrollFactor(0).setScale(2.5).setDepth(250).setInteractive().on('pointerdown', (pointer, localX, localY, event)=>{
            this.LeftArrowDown=true;
        }).on('pointerup', (pointer, localX, localY, event)=>{
            this.LeftArrowDown=false;
        });
        scene.add.image(235,295,"Right_Button").setScrollFactor(0).setScale(2.5).setDepth(250).setInteractive().on('pointerdown', (pointer, localX, localY, event)=>{
